...
BOT_TOKEN = "6359606403:AAE0szW-PCuMwHS0ppjQ9ObzbYoZh_Gcjjw"
...
async def message_to_admin(message: types.Message):
    await message.answer("Habaringizni qoldiring:")
    await AdminComment.comment.set()


@dp.message_handler(state=AdminComment.comment)
async def admin_state(message: types.Message, state: FSMContext):
    await state.update_data(comment=message.text)
    print(f"@{message.from_user.username} nomli foydalanuvchidan, komentariya keldi: <<{message.text}>>")

    await message.answer("Raxmat, sizning fikringiz biz uchun muhim!")
    await state.finish()
